...
- 토큰 갱신 실패 + 잔여 2일 미만
- **주 1회 생존 신호**(`HEARTBEAT_CRON`, 기본 월요일 09:00) — 토큰 잔여 일수와 큐 잔량

생존 신호가 필요한 이유는 앞의 두 알림이 스케줄러가 살아 있어야만 울리기 때문입니다.
프로세스가 죽으면 발행 시도 자체가 없어 실패도 없고, 따라서 알림도 영영 오지 않습니다.
**월요일에 신호가 안 오면 스케줄러가 죽은 것입니다.**
...
- **하루 상한**: `MAX_POSTS_PER_DAY`(기본 1). PC가 꺼져 있던 날의 밀린 글은 하루 1건씩 소화됩니다. 댓글이 중간에 실패한 `partial`도 메인 글은 이미 올라갔으므로 1건으로 셉니다.
- **당일 누락분 따라잡기**: 스케줄러가 뜰 때 **오늘 예정 시각이 이미 지났는데 오늘 발행이 0건이면** 즉시 1건 올립니다. PC가 절전이면 cron 타이머가 멈추고 깨어나도 지나간 시각을 소급 실행하지 않아서입니다. **같은 날 안에서만** 따라잡습니다 — 다음 날 새벽에 부팅하면 그날 예정 시각이 아직 미래라 따라잡지 않고 정규 회차에 맡깁니다(도달 없는 새벽 발행 방지, ADR-017).
...
55개 통과. 검증(글자 수·금칙어·필수 필드·replies 계약), 이력(중복·KST 일일 카운트·partial 집계), 큐 파싱(JSON·Markdown·정렬·미래 publishAt·깨진 파일·front-matter 덮어쓰기 방지), 락 나이 판정, 토큰 만료 계산, 알림 훅, 발행 조립 라인(메인+연쇄 댓글 사슬·partial·재시도 정책), 그리고 Threads API 계약 테스트(`reply_to_id`·`creation_id` 파라미터명, 오류 정규화)를 덮습니다.
