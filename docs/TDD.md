...
## 13. 미결 사항
1. 댓글 중간 실패 시 자동 이어쓰기(published k개 건너뛰고 재개)를 1차에 넣을지 — 구현 난이도는 낮으나 이력 구조가 복잡해짐
2. `link_attachment` 사용 시 미리보기 카드가 뜨는지, 본문 내 URL과 중복 표기되는지 실 테스트로 확인 필요
3. 알림 훅(P1)의 구현 채널 — 카카오톡 나에게 보내기 API vs SMTP
